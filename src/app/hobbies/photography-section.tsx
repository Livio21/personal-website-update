...
export function PhotographySection() {
  ...
  useEffect(() => {
    const fetchPhotos = async () => {
      setIsLoading(true);
      try {
        const unsplashPhotos = await getPhotosByUsername(
          process.env.NEXT_PUBLIC_UNSPLASH_USERNAME || "l1v1o"
        );
        const formattedPhotos: ImagePlaceholder[] = unsplashPhotos.map(
          (p: UnsplashPhoto) => ({
            id: p.id,
                  ...
                  <DialogDescription className="text-left">
                    {selectedImage.unsplashUrl && (
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        className="bg-card/20 backdrop-blur-sm border-white/20 text-white hover:bg-card/50 hover:text-white mt-2"
                      >
                        <Link
