    ...
    <section className="h-full w-full snap-start flex-shrink-0 flex flex-col items-center justify-center p-8 md:p-16">
      ...
      <div className="flex-grow w-full max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        ...
        <div className='group flex flex-col items-center text-center'>
            ...
            <Card className="w-full max-w-xs overflow-hidden bg-card/40 backdrop-blur-sm border-none aspect-square shadow-lg transition-transform duration-300 ">
                <Image
            ...
            <div className="relative group w-full max-w-xs aspect-square">
                <div className="grid grid-cols-2 grid-rows-2 gap-2 h-full">
                    {rotationPreview.map((track, index) => (
                        <Card key={track.name + index} className="overflow-hidden bg-card/40 backdrop-blur-sm border-none shadow-lg group/item">
          ...
          <ScrollArea className="h-full pr-4 -mr-2">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 py-4">
                {otherTracks.map((track, index) => (
                    <Card key={track.name+index} className="group overflow-hidden bg-card/40 backdrop-blur-sm border-none aspect-square shadow-lg transition-transform duration-300 rounded-md">
                        <CardContent className="p-0 w-full h-full relative">
                        <Image
                            src={track.image}
                            alt={`Album art for ${track.name} by ${track.artist}`}
